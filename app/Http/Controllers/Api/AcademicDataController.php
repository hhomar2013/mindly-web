...
class AcademicDataController extends Controller
{
    ...
    public function getAcademicStructure()
    {
        ...
        // $transformedData = Cache::remember('academic_structure_data', 86400, function () {
        //     $systems = EducationSystem::with([
        //         'stages' => function ($query) {
        //             $query->with([
        //                 'grades',
        ...
        // 1. بنخزن فقط المصفوفة الناتجة من التحويل (Array)
        $transformedData = Cache::remember('academic_structure_data', 86400, function () {
            // التحميل المتعجل لجميع العلاقات
            $systems = EducationSystem::with([
                'stages.grades',
                'stages.tracks.grades',
                'stages.tracks.branches.subBranches',
                'stages.faculties',
                'stages.institutes',
                'stages.universityAcademicYears'
            ])->orderBy('id')->get();

            // تنفيذ دالة التحويل وتخزين نتيجتها (مصفوفة نظيفة)
            return $this->transformSystems($systems);
        });
