<div class="row p-4">
    ...
    <div class="col-lg-6 text-end">
        ...
        @if (!$lesson_id)
        <button class="btn btn-danger btn-rounded" wire:click="back('subject-managment','course_id' ,false,['lesson_content_ar_name','lesson_content_en_name','content_type_id','lesson_content_link','image'])">
            <i class="fas fa-arrow-left"></i>
        </button>
        @else
        <button class="btn btn-danger btn-rounded" wire:click="subjectManagment({{ $subjectShow->id }})"> <i class="fas fa-arrow-left"></i>
        </button>
        @endif
    </div>
                ...
                <div class="card-body text-start">
                    <div class="row">
                        <div class="col-lg-4">
                            ...
                            <input type="text" wire:model="lesson_content_ar_name" class="form-control" id="lesson_name">
                            @error('lesson_content_ar_name')
                            <span class="text-danger"> {{ $message }} </span>
                            @enderror
                            <label for="lesson_name"> {{ __('English Name') }} </label>
                            <input type="text" wire:model="lesson_content_en_name" class="form-control" id="lesson_name">
                        ...
                        <div class="col-lg-4">
                            ...
                            <select name="content_type_id" id="content_type_id" wire:model.live="content_type_id" wire:change="getSelectedContentType" class="form-control">
                            ...
                            @elseif($selectedContentType == 'document')
                            <label for="">{{ __('Select Document') }}</label>
                            <input type="file" class="form-control rounded-lg" wire:model="lesson_content_document" />
                            @endif
                        </div>
